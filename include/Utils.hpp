#ifndef _UTILS_H_
#define _UTILS_H_

#include <string>

namespace Utils
{
    /**
     * @brief Generate new random seed (based on time)
     */
    void newRandomSeed();

    /**
     * @brief Get a random number between 0 and max
     *
     * @param[in] max Max random number
     * @return int
     */
    int getRandomNumber(int max);

    /**
     * @brief Sleep for X milliseconds
     *
     * @param[in] milliseconds Sleeping duration in milliseconds
     */
    void sleep(int milliseconds);

    /**
     * @brief Test if a string is a number
     *
     * @param[in] s String to test
     * @return true if the string is a number
     * @return false otherwise (letters, special chars, empty, ...)
     */
    bool isNumber(const std::string &s);

    /**
     * @brief Trim a string
     * @note This function remove leading and trailing spaces
     *
     * @param[in] str Input string
     * @param[in] whitespace Space to remove
     * @return std::string result
     */
    std::string trim(const std::string &str, const std::string &whitespace = " \t");

    /**
     * @brief Reduce a string
     * @note This function remove leading and trailing spaces and replace spaces in string by content of fill argument
     *
     * @param[in] str Input string
     * @param[in] fill String to replace whitespace in middle of input string
     * @param[in] whitespace Space to remove
     * @return std::string
     */
    std::string reduce(const std::string &str, const std::string &fill = " ", const std::string &whitespace = " \t");

} // namespace Utils

#endif /* _UTILS_H_ */
